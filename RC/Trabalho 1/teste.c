...
typedef enum {
    DISCONNECTED,   /* antes do SET                      */
    CONNECTED,      /* após UA enviado                   */
    DISCONNECTING   /* após DISC recebido, à espera de UA */
} conn_state_t;
...
int main(int argc, char *argv[])
{
    ...
    int data_index  = 0;
    int expectedSeq = 0;   /* próximo número de sequência esperado (0 ou 1) */

    /* ----------------------------------------------------------------
     * Loop principal de receção — trata todos os estados do protocolo
     * ---------------------------------------------------------------- */
    while (1) {
        ...
        switch (state) {
            ...
            /* ------ FLAG_RCV ------ */
            case FLAG_RCV:
                if (byte == FLAG) { /* FLAGS consecutivos OK */ }
                else if (byte == A_TX_CMD) { A = byte; state = A_RCV; }
                else {
            ...
            case A_RCV:
                if (byte == FLAG) {
                    ...
                    /* UA do transmissor a confirmar a desconexão */
                    C = byte; state = C_RCV;
                } else {
            ...
            case BCC1_OK:
                if (byte == FLAG) {
                    /* Trama de supervisão / controlo (sem dados) */
                    if (C == C_SET) {
                        ...
                        /* Responder com DISC e entrar em DISCONNECTING */
                        ...
                        goto cleanup;   /* terminar normalmente */
                    ...
                    /* Início de payload de I-frame */
                    if (connState != CONNECTED) {
                        printf("Data before connection. Dropping.\n"); fflush(stdout);
                        state = START; data_index = 0;
                ...
                if (byte == FLAG) {
                    ...
                    if (bcc2_read == bcc2_calc) {
                        if (frameSeq == expectedSeq) {
                            /* Trama nova e correta → avançar seq, enviar RR */
                            ...
                            /* Duplicado correto → reenviar último RR sem avançar */
                            unsigned char rrC = (expectedSeq == 1) ? C_RR1 : C_RR0;
                        ...
                        if (frameSeq == expectedSeq) {
                            ...
                            /* Duplicado com erro → repetir último ACK */
                            unsigned char rrC = (expectedSeq == 1) ? C_RR1 : C_RR0;
